...
<div class="container-fluid">
    ...
    <div class="col-sm-9" style="background-color:#e2d9d9;">
        ...
        <h3 style="padding-top:8%; text-shadow: .1rem .1rem .5rem black;">All TRANSCRIPT PAYMENTS</h3>
    ...
    ?>
                    ...
                    <table class="table table-striped" style="width:95%; font-size:12px; overflow-x:scroll; margin-right: 10%; margin-top:30px; margin-left:5%;background:#fff;">
                        ...
                        <?php 
                         if (isset($_GET['searchbutton'])){
                                $search = $_GET['search'];
                                $query = mysqli_query($conn, "SELECT * FROM transcript_applications WHERE surname LIKE '%$search%' OR firstname LIKE '%$search%' OR AppRef LIKE '%$search%' OR RecRef LIKE '%$search%' OR formDelivery LIKE '%$search%' OR Location LIKE '%$search%' OR DateApplied LIKE '%$search%'");
                              }
                              else{
                    ...
                    while($result=mysqli_fetch_assoc($query)){
                        ...
                        <tr>
                            ...
                            <td><?php echo $result['DatePaid'];?></a></td>
                            <td><?php if ($result['ProcessingStatus']=='0') {?>
                                <a onclick = "return confirm('Click Ok to Confirm?')" href="adminact.php?iid=<?php echo $result["AppRef"]; ?>"><span class="btn btn-success btn-sm">Confirm</span></a>
                            <?php  } if ($result['ProcessingStatus']=='1') {?>
                                <a onclick = "return confirm('Any Issue, Click Ok to cancel Confirmation?')" href="adminact.php?idis=<?php echo $result["AppRef"];  ?>"><span class="btn btn-warning btn-sm ">Cancel</span></a>
