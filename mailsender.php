...
<div class="container-fluid">
    ...
    <div class="col-sm-9" style="background-color:#e2d9d9;">
        ...
        <h3 style="padding-top:8%; text-shadow: .1rem .1rem .5rem black;">MAIL RECIPIENTS</h3>
    ...
    ?>
          <div class="table-responsive">
                    ...
                    <table class="table table-striped table-bordered" style="width:98%; font-size:12px; overflow-x:auto; margin-right: 10%; margin-top:30px; margin-left:3%;background:#fff;">
                        ...
                        $conns =new PDO("mysql:host=localhost;dbname=transcripts", "root", "");
                        $postid=$_GET['dell'];
                        $querys = mysqli_query($conn, "SELECT subject, mailbody, filename FROM mailmessage WHERE id='$postid'");
                
                        $resultss = mysqli_fetch_array($querys);
                        
                         if (isset($_GET['searchbutton'])){
                                $search = $_GET['search'];
                                $_GET['dell'];
                                $query ="SELECT * FROM transcript_applications WHERE RegNo LIKE '%$search%' OR faculty LIKE '%$search%' OR surname LIKE '%$search%' OR onames LIKE '%$search%' OR AppRef LIKE '%$search%' OR RecRef LIKE '%$search%' OR formDelivery LIKE '%$search%' OR location LIKE '%$search%' OR DateApplied LIKE '%$search%'";
                              }
                              else{
                                $query="SELECT * FROM transcript_applications ORDER BY DateApplied desc";}
                                $sstat =$conns->prepare($query);
                                $sstat->execute();

                                $result=$sstat->fetchAll();
                    $srno=0;
                    foreach ($result as $results) {
                      ...
                      echo '
                        <tr>
                            ...
                            <td>'.$results["AppRef"].'</td>
                            <td>'.$results["RegNo"].'</td>
                            <td>'.$results["surname"].' '. $results["onames"].'</td>
                            <td>'.$results["AppEmail"].'</td>
                            <td>'.$results["formDelivery"].'</td>
                            <td>'.$results["location"].'</td>
                            <td><input type="checkbox" name="single_select" class="single_select" data-email="'.$results["AppEmail"].'" data-subject="'.$resultss['subject'].'" data-body="'.$resultss['mailbody'].'" data-file="'.$resultss['filename'].'" data-name="'.$results['surname'].' '.$results['onames'].'"/></td>
                            
                            
                            
                            
                            <td> <button type="button" name="email_button" class="btn btn-info btn-xs email_button" id="'.$srno.'" data-email="'.$results["AppEmail"].'" data-subject="'.$resultss['subject'].'" data-body="'.$resultss['mailbody'].'" data-file="'.$resultss['filename'].'" data-name="'.$results['surname'].' '.$results['onames'].'" data-action="single"><span>Send</span> </a>
                            </td>
                        </tr>
                        ';
                      }
                      ?>
                          ...
                          <tr>
                            ...
                            <td> <button type="button" name="bulk_email" class="btn btn-info btn-xs email_button" id="bulk_email" data-action="bulk"><span>Send Bulk</span></button>
                            </td>
                        </tr>      
