...
<div class="container-fluid">
    ...
    <div style="margin-right:3%;width:80%">
     <h5 style="color:#fff; font-size: 15px;margin-left: 8%">Name: <?php echo $_SESSION["surname"];?> <?php echo $_SESSION["firstname"];?></h5>
         ...
         <ul>
             ...
             <li><a href="processstranscr.php">Processed Transcript</a></li>
    ...
    <div class="col-sm-9" style="background-color:#e2d9d9;">
        ...
        <h3 style="padding-top:2rem; text-shadow: .1rem .1rem .5rem black; padding-top: 10%">Processed Transcript</h3>
    ...
    ?>
          <div style="width:35%;float:right">
                    ...
                    <table class="table table-striped" style="width:90%; content-align:center; font-size:12px; overflow-x: scroll; margin-right: 10%; margin-top:30px; margin-left:10%;background:#fff;">
                        ...
                        <?php 
                         if (isset($_GET['searchbutton'])){
                                $search = $_GET['search'];
                                $query = mysqli_query($conn, "SELECT * FROM payment_process WHERE RegNo LIKE '%$search%' OR surname LIKE '%$search%' OR firstname LIKE '%$search%' OR PhoneNO LIKE '%$search%' OR regdate LIKE '%$search%'");
                              }
                              else{
                                $query=mysqli_query($conn, "SELECT * FROM payment_process ORDER BY datePaid desc");}
                    ...
                    while($result=mysqli_fetch_assoc($query)){
                        ...
                        <tr>
                            ...
                            <td><?php if ($result['status']=='0') {?>
                                <a onclick = "return confirm('Are you sure you want to Suspend User Acces?')" href="adminact.php?id=<?php echo $id; ?>"><span class="btn btn-warning btn-sm">Not Sign</span></a>
                            <?php  } if ($result['status']=='1') {?>
                                <a onclick = "return confirm('Are you sure you want to Grant User Access?')" href="adminact.php?dis=<?php echo $id; ?>"><span class="btn btn-success btn-sm ">Signed</span></a>
